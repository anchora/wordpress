...
function wpjam_custom_upload_dir( $uploads ) {
	...
	$upload_path = 'img';
	$upload_url_path = home_url();
	if ( empty( $upload_path ) || 'wp-content/uploads' == $upload_path ) {
		$uploads['basedir']  = WP_CONTENT_DIR . '/uploads';
	} elseif ( 0 !== strpos( $upload_path, FILEPATH ) ) {
		$uploads['basedir'] = path_join( FILEPATH, $upload_path );
	} else {
	...
	if ( $upload_url_path ) {
		...
		$uploads['url'] = $uploads['basedir'].$uploads['subdir'] ;//$uploads['baseurl']."/".$upload_path.$uploads['subdir'];
	}	
