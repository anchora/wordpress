...
function wp_get_theme( $stylesheet = null, $theme_root = null ) {
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root )
			...
			$theme_root = FILEPATH . '/themes';
		elseif ( ! in_array( $theme_root, (array) $wp_theme_directories ) )
			//$theme_root = WP_CONTENT_DIR . $theme_root;
			$theme_root = FILEPATH . $theme_root;
	}
...
function get_theme_root_uri( $stylesheet_or_template = false, $theme_root = false ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $wp_theme_directories ) ) {
			...
			$theme_root_uri = get_theme_root($theme_root);
			//$theme_root_uri = content_url( $theme_root );
		}
	} else {
		//主题目录
		$theme_root_uri = get_theme_root();
...
<?php		endif; ?>
		...
		jQuery(function(){
			...
			jQuery("img").each(function(){
				var srcContent = jQuery(this).attr("src"); 
				if(srcContent.indexOf("<?php echo esc_url(get_template_directory_uri());?>") !== -1){
					jQuery(this).attr("src",'<?php echo home_url()."/showthumbnail.php?path=";?>'+base64encode(srcContent));
				}
			});
		});
	</script>
